...
module.exports = class Menu extends EventEmitter {
     ...
     */
    constructor(channel, userId, pageArray, timeout = 60 * 1000, idle = 15 * 1000) {
        super()
    ...
    async start() {
        ...
        const menu = await this.channel.send({ embeds: [this.currentPage.content], components: this.currentPage.actionRows })
        this.menu = menu
        this.startButtons()
    }
     ...
     */
    async startButtons() {
        const filter = i => i.interaction.user.id === this.userId
